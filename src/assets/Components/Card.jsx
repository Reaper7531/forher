...
function Card({ name, img, note, color }) {
  return (
    <div
      className={`flex h-80 w-72  flex-col rounded-2xl border border-pink-800 shadow-xl  ${
    ...
    >
      <div className="flex h-1/4 bg-black rounded-t-2xl justify-between">
        ...
        <div className="flex h-full w-3/4 items-center justify-center">
          <h1 className=" text-xl font-bold text-pink-400 card-font">
            - {name}
          </h1>
        </div>
      </div>
      <div className="flex h-3/4 justify-center  items-end">
        <p className="w-[90%] h-[90%]  font-sans whitespace-pre-wrap text-md overflow-y-auto text-black">
          {note}
        </p>
      </div>
    </div>
